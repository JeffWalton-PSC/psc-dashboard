...
"""
        ...
        if year_start and year_end and term and gpa_type:
            ...
            atgpa = academic.merge(transcript_gpa,
                how='inner',
                on=['PEOPLE_CODE_ID', 'yearterm', 'yearterm_sort'] 
                )
            ...
            atgpa = atgpa.sort_values(['id', 'yearterm_sort'])
            atgpa['term'] = atgpa.groupby(['id'])['yearterm_sort'].rank(method='dense', ascending=True)
            # atgpa = atgpa.drop(columns=['yearterm', 'yearterm_sort'])
            ...
            atgpa_transposed = atgpa_transposed.rename(columns=lambda x: f'term{int(x):02d}')
